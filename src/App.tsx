...
const HamburgerButton: FC<{ onClick: () => void; isOpen: boolean }> = ({ onClick, isOpen }) => (
  <button 
    onClick={onClick} 
...
const BrandButton: FC<{ onClick: () => void; visible: boolean; brandName: string }> = ({ onClick, visible, brandName }) => (
  <button 
    onClick={onClick} 
  ...
  >
     <h1 className="font-serif text-2xl font-bold text-white tracking-tighter">{brandName}.</h1>
  </button>
);
...
const FadeInItem: FC<FadeInItemProps> = ({ children, delay = 0, className = "" }) => {
...
const LoadingView: FC = () => (
  <div className="min-h-screen flex items-center justify-center bg-paper">
    <div className="text-center">
      ...
      <p className="font-sans text-xs tracking-[0.3em] uppercase text-muted">Loading Content...</p>
    </div>
  </div>
);
...
const HomeView: FC<HomeViewProps> = ({ items, siteTitle, siteDescription, onItemClick }) => {
  const rightColRef = useRef<HTMLDivElement>(null);
  
  const feedItems = items.filter(
    item => item.type === ContentType.STORY || item.type === ContentType.POEM
  );
  ...
  return (
    <div className="min-h-screen relative bg-paper overflow-hidden">
      <div className="max-w-[1600px] mx-auto px-6 md:px-12 pt-40 md:pt-64 pb-32 flex flex-row gap-8 md:gap-32">
         <div className="w-1/2 flex flex-col pt-12">
            <div className="mb-48 md:mb-64">
               <h1 className="font-serif text-6xl md:text-8xl lg:text-9xl leading-[0.9] tracking-tighter mb-12 animate-slide-down">
            ...
            {leftColItems.map((item, idx) => (
              <Card key={item.id} item={item} offsetIndex={idx * 2} onItemClick={onItemClick} />
            ))}
         </div>
         <div ref={rightColRef} className="w-1/2 flex flex-col will-change-transform pt-32 md:pt-96">
            {rightColItems.map((item, idx) => (
              <Card key={item.id} item={item} offsetIndex={(idx * 2) + 1} onItemClick={onItemClick} />
            ))}
         </div>
      </div>
    </div>
...
const FilteredListView: FC<FilteredListViewProps> = ({ type, items, onItemClick }) => {
  const typeLabel = type === ContentType.POEM ? 'Poetry' : type === ContentType.STORY ? 'Stories' : `${type}s`;
  
  return (
    <div className="min-h-screen pt-40 pb-32 px-6 md:px-32 max-w-6xl mx-auto opacity-0 animate-fade-in">
      <header className="mb-32 flex flex-col items-start animate-fade-in-up">
        <span className="font-sans text-xs tracking-[0.2em] text-muted uppercase mb-4">Collection</span>
        <h1 className="font-serif text-6xl md:text-8xl mb-6 text-dark capitalize tracking-tight">
          {typeLabel}
        </h1>
            ...
            >
              {item.type === ContentType.QUOTE ? (
                <figure className="max-w-4xl">
                  <blockquote className="font-serif text-3xl md:text-5xl leading-tight group-hover:text-gray-600 transition-colors duration-300">
                ...
                <article>
                  ...
                  <h2 className="font-serif text-4xl md:text-5xl mb-6 group-hover:text-accent transition-colors duration-300">
                    {item.title}
                  </h2>
                  ...
                  <div className="inline-block overflow-hidden relative">
                    <span className="block font-sans text-xs font-bold tracking-widest uppercase transform group-hover:-translate-y-full transition-transform duration-300">Read Piece</span>
                    <span className="absolute top-full left-0 block font-sans text-xs font-bold tracking-widest uppercase text-accent transform group-hover:-translate-y-full transition-transform duration-300">Read Piece</span>
                  </div>
                </article>
              )}
            </div>
          );
          if (type === ContentType.QUOTE) return <div key={item.id}>{content}</div>;
          return <FadeInItem key={item.id} delay={index * 50}>{content}</FadeInItem>;
        })}
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-paper animate-slide-down relative z-30">
      <div className="max-w-3xl mx-auto pt-32 px-8 pb-32">
        <div className="flex justify-between items-center mb-20">
          <button onClick={onBack} className="group flex items-center space-x-3 text-xs font-bold tracking-[0.2em] uppercase text-muted hover:text-dark transition-colors">
            <span className="transform group-hover:-translate-x-1 transition-transform duration-300">&larr;</span>
            <span>Return</span>
          </button>
          <button onClick={handleShare} className="text-xs font-bold tracking-[0.2em] uppercase text-muted hover:text-dark transition-colors">
            {shared ? 'Link Copied' : 'Share'}
          </button>
        </div>
        <article className="animate-fade-in-up" style={{ animationDelay: '0.3s' }}>
          <header className="mb-16 text-center">
            {item.date && <span className="block font-sans text-xs tracking-widest text-accent mb-6 uppercase">{item.date}</span>}
            {item.title && <h1 className="font-serif text-5xl md:text-7xl leading-none mb-10 text-dark">{item.title}</h1>}
            <div className="h-px w-24 bg-gray-200 mx-auto"></div>
          </header>
          <div className="prose prose-xl prose-p:font-serif prose-p:text-gray-600 prose-p:leading-loose mx-auto first-letter:float-left first-letter:text-7xl first-letter:pr-4 first-letter:font-serif first-letter:text-dark">
             {item.body.split('\n').map((paragraph, idx) => <p key={idx} className="mb-8">{paragraph}</p>)}
          </div>
        </article>
      </div>
    </div>
...
type ViewType = 'HOME' | typeof ContentType[keyof typeof ContentType] | 'CMS' | 'LOGIN';
...
function App() {
  ...
  const { items, settings, isLoading, isGitHubConfigured, saveItem, deleteItem, saveSettings } = useGitHubDB();
  ...
  const handleLoginSubmit = (password: string): boolean => {
  ...
  return (
    <div className="bg-paper min-h-screen relative overflow-x-hidden">
      ...
      {/* Full Screen Menu */}
      <div className={`fixed inset-0 bg-dark z-40 transform transition-transform duration-700 ease-[cubic-bezier(0.87,0,0.13,1)] ${menuOpen ? 'translate-y-0' : '-translate-y-full'}`}>
        <div className="h-full w-full max-w-[1600px] mx-auto flex flex-col md:flex-row items-center justify-center md:justify-between px-8 md:px-24">
            <div className={`md:w-1/3 text-center md:text-left transition-all duration-700 delay-300 ${menuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            ...
            <nav className="flex flex-col items-center md:items-end justify-center space-y-6 mt-16 md:mt-0">
              {[
